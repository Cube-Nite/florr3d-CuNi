...
const setMerge = (res, token) => res.appendHeader('Set-Cookie',
  `merge=${token}; Path=/; Max-Age=${MERGE_TTL_MS / 1000}; HttpOnly; Secure; SameSite=Lax`);
const clearMerge = (res) => res.appendHeader('Set-Cookie',
  'merge=; Path=/; Max-Age=0; HttpOnly; Secure; SameSite=Lax');
      ...
      try {
        ...
        if (guestSave) {
          ...
          if (!discordSave) {
            ...
            return redirect(res, '/?merge=1'), true;
          }
        } else if (wasGuest) {
